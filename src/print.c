...
/*   Updated: 2021/03/04 20:45:08 by helvi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((info->cursorcol + (int)ft_strlen(printable->name) + 4)
		> info->screencols)
		print_mv_cursor("\r\n", info);
...
void		print_selections(t_terminal *info, t_option *first)
{
	...
	ft_putstr_fd("\r\n", info->fd_out);
	tputs(tgoto(info->cm_string, info->cursor_saved_col,
		info->cursor_saved_row), info->screenrows, &ft_putc);
}
